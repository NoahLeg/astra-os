...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background"><Sidebar /><div className={cn("min-h-screen transition-[margin] duration-300", sidebarCollapsed ? "lg:ml-[76px]" : "lg:ml-[248px]")}><Topbar /><main className={cn("mx-auto w-full max-w-[1600px]", account?.preferences?.density === "compact" ? "p-3 md:p-4 xl:p-5" : "p-4 md:p-6 xl:p-8")}>{children}</main></div><AssistantPanel /><CommandPalette /></div>;
}
