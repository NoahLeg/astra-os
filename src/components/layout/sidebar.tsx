...
export function Sidebar() {
  ...
  return (
    <nav className={cn("fixed inset-y-0 left-0 z-50 hidden flex-col overflow-hidden transition-[width] duration-300 lg:flex glass-panel", sidebarCollapsed ? "w-[80px]" : "w-[264px]")}>
      ...
      <div className="p-3">
        {hasAccess(account?.accessLevel, "operator") ? <Link href="/goals/new" className={cn("flex h-10 items-center justify-center gap-2 rounded-lg text-sm font-semibold shadow-[var(--shadow-floating)] transition-all duration-[var(--duration-normal)] ease-[var(--ease-out-expo)] active:scale-[.97] glass-lens text-primary-foreground", sidebarCollapsed ? "px-0" : "px-3")}><Plus className="size-4" />{!sidebarCollapsed ? <span>Nouvel objectif</span> : null}</Link> : null}
      </div>
      ...
      <nav className="scrollbar-none flex-1 space-y-0.5 overflow-y-auto px-3 pb-4">
        ...
        {visibleRoutes.map((route) => {
          ...
          return <Link key={route.href} href={route.href} title={sidebarCollapsed ? route.label : undefined} className={cn("group relative flex h-9 items-center gap-3 rounded-lg px-3 text-sm transition-all duration-[var(--duration-fast)] ease-[var(--ease-out-expo)] hover:translate-x-0.5", active ? "glass-lens h-10 my-0.5 text-primary-foreground font-medium" : "text-muted-foreground hover:bg-muted hover:text-foreground", sidebarCollapsed && "justify-center px-0 hover:translate-x-0")}>
            <DynamicIcon name={route.icon} className={cn("size-4 shrink-0 transition", active ? "text-primary-foreground" : "text-muted-foreground group-hover:text-foreground")} />
            {!sidebarCollapsed ? <span>{route.label}</span> : null}
            {active && !sidebarCollapsed ? <span className="absolute left-0 size-1 rounded-full bg-primary-foreground ring-2 ring-primary-foreground/30" /> : null}
            {route.href === "/approvals" && !sidebarCollapsed && pendingApprovals > 0 ? <span className="ml-auto rounded-full border border-primary-foreground/30 bg-primary-foreground/10 px-2 py-0.5 font-mono text-[9px] font-medium text-primary-foreground">{pendingApprovals}</span> : null}
          </Link>;
        })}
      </nav>
