...
export function Topbar() {
  ...
  return (
    <header ref={headerRef} className={cn("sticky top-0 z-40 flex h-[72px] items-center gap-1 border-b bg-[var(--glass-bg-strong)] px-2 shadow-[var(--shadow-sm)] backdrop-blur-[var(--glass-blur-strong)] saturate-[var(--glass-saturate)] transition-shadow duration-[var(--duration-normal)] sm:gap-3 sm:px-4 md:px-6", scrolled ? "border-[var(--glass-border)] shadow-[var(--shadow-md)]" : "border-transparent")}>
      ...
      <button type="button" onClick={() => setCommandOpen(true)} className="mx-auto flex h-10 min-w-0 flex-1 items-center gap-2 rounded-lg border border-[var(--glass-border)] bg-[var(--glass-bg)] px-2 text-left text-sm text-muted-foreground shadow-[var(--shadow-sm)] backdrop-blur-[var(--glass-blur)] transition-all duration-[var(--duration-fast)] ease-[var(--ease-out-expo)] hover:border-primary/40 hover:shadow-[var(--shadow-md)] sm:max-w-[460px] sm:px-3"><Search className="size-4 shrink-0 text-primary" /><span className="min-w-0 flex-1 truncate"><span className="sm:hidden">Rechercher…</span><span className="hidden sm:inline">Rechercher un objectif, agent, document…</span></span><kbd className="hidden rounded border border-[var(--glass-border)] bg-muted px-1.5 py-0.5 font-mono text-[9px] md:inline">Ctrl K</kbd></button>
