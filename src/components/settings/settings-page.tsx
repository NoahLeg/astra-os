    ...
    <div className="space-y-7">
      ...
      <div className="grid gap-5 lg:grid-cols-[230px_1fr]">
        ...
        <div className="space-y-5">
          ...
          {section === "models" && <Card><CardHeader><CardTitle>Modèles IA</CardTitle></CardHeader><CardContent className="space-y-3">{models.map((model) => <div key={model.id} className="flex items-center gap-4 rounded-xl border bg-background p-4"><div className="flex-1"><div className="flex items-center gap-2"><p className="font-medium">{model.name}</p><Badge className="bg-muted text-muted-foreground">{model.provider}</Badge>{settings.defaultModelId === model.id && <Badge className="bg-indigo-500/10 text-indigo-500">Par défaut</Badge>}</div><p className="mt-1 text-xs text-muted-foreground">Fenêtre de contexte : {model.contextWindow}</p></div><Button variant="ghost" size="sm" disabled={!enabledModels.has(model.id)} onClick={() => update("defaultModelId", model.id)}>Utiliser par défaut</Button><Switch checked={enabledModels.has(model.id)} onCheckedChange={(checked) => update("enabledModelIds", checked ? [...settings.enabledModelIds, model.id] : settings.enabledModelIds.filter((id) => id !== model.id))} label={`Activer ${model.name}`} /></div>)}</CardContent></Card>}
