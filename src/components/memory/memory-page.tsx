...
function KnowledgeGraph({ items, onSelect }: { items: MemoryItem[]; onSelect: (item: MemoryItem) => void }) {
  ...
  return <Card className="relative min-h-[520px] overflow-hidden soft-grid"><CardContent className="relative min-h-[520px] p-6"><svg className="absolute inset-0 h-full w-full opacity-30" aria-hidden="true"><line x1="50%" y1="50%" x2="20%" y2="25%" stroke="#6366f1" /><line x1="50%" y1="50%" x2="78%" y2="22%" stroke="#06b6d4" /><line x1="50%" y1="50%" x2="25%" y2="78%" stroke="#8b5cf6" /><line x1="50%" y1="50%" x2="76%" y2="76%" stroke="#10b981" /></svg><div className="absolute left-1/2 top-1/2 z-10 flex size-28 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full border border-indigo-500/40 bg-card shadow-2xl shadow-indigo-500/20"><BrainCircuit className="size-6 text-indigo-500" /><span className="mt-1 text-xs font-medium">Votre contexte</span></div>{items.slice(0, 6).map((item, index) => <button key={item.id} onClick={() => onSelect(item)} className={cn("absolute z-10 max-w-[180px] rounded-2xl border bg-card p-3 text-left shadow-lg transition hover:scale-105 hover:border-indigo-500/50", positions[index], item.blocked && "opacity-40")}><div className="flex items-center gap-2"><GitBranch className="size-3.5 text-cyan-500" /><Badge className="bg-muted text-muted-foreground">{item.type}</Badge></div><p className="mt-2 text-xs font-medium">{item.title}</p><p className="mt-1 line-clamp-2 text-[10px] leading-4 text-muted-foreground">{item.content}</p></button>)}</CardContent></Card>;
}
