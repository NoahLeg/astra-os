          ...
          <Card key={agent.id} className="group">
            <CardContent className="p-5">
              <div className="flex items-start justify-between">
                ...
                <Switch checked={agent.enabled} onCheckedChange={() => { toggleAgent(agent.id); toast.success(`${agent.name} ${agent.enabled ? "mis en pause" : "activé"}`); }} label={`${agent.enabled ? "Désactiver" : "Activer"} ${agent.name}`} />
              </div>
...
export function AgentDetailPage({ id }: { id: string }) {
  ...
  const [result, setResult] = useState<{ content: string; confidence: number; model: string }>();
  ...
  const run = async () => {
    ...
    try {
      ...
      setResult({ content: execution.result, confidence: execution.confidence, model: execution.model });
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border bg-card p-6 md:p-8"><div className="flex flex-col gap-5 md:flex-row md:items-center"><span className="flex size-16 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-cyan-400 text-white shadow-xl shadow-indigo-500/20"><Bot className="size-7" /></span><div className="flex-1"><div className="flex items-center gap-3"><h1 className="text-2xl font-semibold">{agent.name}</h1><AgentStatus status={agent.status} /></div><p className="mt-1 text-sm text-indigo-500">{agent.role}</p><p className="mt-3 max-w-2xl text-sm text-muted-foreground">{agent.description}</p></div><Button variant={agent.enabled ? "outline" : "default"} onClick={() => toggleAgent(agent.id)}>{agent.enabled ? "Mettre en pause" : "Activer l’agent"}</Button></div></section>

      <Card className="border-indigo-500/20"><CardHeader><CardTitle className="flex items-center gap-2"><Play className="size-4 text-indigo-500" />Exécuter une tâche réelle</CardTitle></CardHeader><CardContent><Textarea value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder={`Ex. ${agent.name === "Email" ? "Rédige un email de prospection pour un cabinet comptable." : "Analyse cet objectif et propose les prochaines étapes."}`} className="min-h-28" /><div className="mt-3 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-muted-foreground">Le modèle produit le contenu. Aucun outil externe n’est utilisé sans connecteur et validation.</p><Button onClick={() => void run()} disabled={running || !agent.enabled}>{running ? <LoaderCircle className="size-4 animate-spin" /> : <Play className="size-4" />}{agent.enabled ? "Lancer l’agent" : "Agent désactivé"}</Button></div>{result && <div className="mt-5 rounded-2xl border bg-background p-5"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><Badge className="bg-emerald-500/10 text-emerald-500">Exécution terminée</Badge><div className="flex items-center gap-3"><ConfidenceIndicator value={result.confidence} compact /><Badge className="bg-muted text-muted-foreground">{result.model}</Badge></div></div><p className="whitespace-pre-wrap text-sm leading-6">{result.content}</p></div>}</CardContent></Card>

      <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]"><div className="space-y-5"><Card><CardHeader><CardTitle>Outils prévus</CardTitle></CardHeader><CardContent className="flex flex-wrap gap-2">{agent.tools.map((tool) => <Badge key={tool} className="bg-muted text-muted-foreground"><Wrench className="size-3" />{tool}</Badge>)}</CardContent></Card><Card><CardHeader><CardTitle>Journaux réels</CardTitle></CardHeader><CardContent className="space-y-3">{logs.length ? logs.map((log) => <div key={log.id} className="rounded-xl border bg-background p-3"><div className="flex items-center gap-3"><CheckCircle2 className="size-4 text-emerald-500" /><div className="min-w-0 flex-1"><p className="truncate text-sm">{log.action}</p><p className="mt-1 text-xs text-muted-foreground">{new Date(log.timestamp).toLocaleString("fr-FR")} · {log.duration}s</p></div><ConfidenceIndicator value={log.confidence} compact /></div><p className="mt-3 line-clamp-3 text-xs leading-5 text-muted-foreground">{log.details}</p></div>) : <p className="py-6 text-center text-sm text-muted-foreground">Aucune exécution pour cet agent.</p>}</CardContent></Card></div><div className="space-y-5"><Card><CardHeader><CardTitle>Statistiques mesurées</CardTitle></CardHeader><CardContent className="space-y-5">{[{ label: "Taux de réussite", value: agent.successRate, icon: CheckCircle2 }, { label: "Exécutions terminées", value: Math.min(100, agent.tasksCompleted), icon: Clock3 }, { label: "Dernière confiance", value: logs[0]?.confidence ?? 0, icon: Cpu }].map((item) => <div key={item.label}><div className="flex items-center justify-between text-sm"><span className="flex items-center gap-2 text-muted-foreground"><item.icon className="size-4" />{item.label}</span><span className="font-mono">{item.value}{item.label === "Exécutions terminées" ? "" : "%"}</span></div><Progress value={item.label === "Exécutions terminées" ? Math.min(100, item.value) : item.value} className="mt-2" /></div>)}</CardContent></Card><Card><CardHeader><CardTitle>Permissions</CardTitle></CardHeader><CardContent className="space-y-3">{agent.permissions.map((permission) => <div key={permission.resource} className="rounded-xl border bg-background p-3"><PermissionBadge permission={permission} /><p className="mt-2 text-xs text-muted-foreground">{permission.actions.join(" · ")}</p></div>)}<p className="flex items-start gap-2 text-xs text-muted-foreground"><Shield className="mt-0.5 size-3.5 text-emerald-500" />Les actions sensibles restent soumises à validation.</p></CardContent></Card></div></div>
    </div>
  );
}
