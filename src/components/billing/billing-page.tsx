...
export function BillingPage() {
  ...
  const openCheckout = async (plan: SubscriptionPlan) => {
    setBusyPlan(plan.id);
    try {
      const result = plan.id === subscription?.planId && subscription.managedByStripe ? await billingService.portal() : await billingService.checkout(plan.id);
  ...
  return (
    <div className="space-y-7">
      <PageHeader eyebrow="Facturation et limites" title="Abonnement" description="Choisissez les capacités disponibles pour votre entreprise et suivez la consommation réelle des appels IA." actions={subscription.managedByStripe ? <Button variant="outline" disabled={busyPlan === "portal"} onClick={() => void openPortal()}>{busyPlan === "portal" ? <LoaderCircle className="size-4 animate-spin" /> : <ExternalLink className="size-4" />}Gérer sur Stripe</Button> : undefined} />
