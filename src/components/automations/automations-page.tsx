...
import type { AgentToolName, Automation, AutomationNode, AutomationRun } from "@/types";
...
const toolOptions: Array<{ id: "auto" | AgentToolName; label: string; agentIds?: string[]; connectionId?: string }> = [
  ...
  { id: "smart_organize_gmail", label: "Organiser intelligemment Gmail", agentIds: ["coordinateur", "email"], connectionId: "gmail" },
...
export function AutomationsPage() {
  const { automations, agents, connections, addAutomation, updateAutomation, deleteAutomation, hydrateFromDatabase } = useAppStore();
  ...
  const [autonomyLevel, setAutonomyLevel] = useState<2 | 3 | 4>(2);
  ...
  const openBuilder = (automation?: Automation) => {
    ...
    setSchedule((automation?.schedule as typeof schedule) ?? "manual");
    setAutonomyLevel(automation?.autonomyLevel === 3 || automation?.autonomyLevel === 4 ? automation.autonomyLevel : 2);
    setMaximumAttempts(automation?.retryPolicy?.maximumAttempts ?? 3);
    setBackoffSeconds(automation?.retryPolicy?.backoffSeconds ?? 2);
    setBuilderOpen(true);
  };
  ...
  const save = async () => {
    ...
    if (["trigger", "agent", "action", "result"].some((type) => !nodes.some((node) => node.type === type))) {
      toast.error("Le workflow doit contenir un déclencheur, un agent, une action et un résultat.");
      return;
    }
    if (["trigger", "agent", "action", "result"].some((type) => nodes.filter((node) => node.type === type).length !== 1) || nodes.filter((node) => node.type === "approval").length > 1) {
      toast.error("Utilisez un seul bloc déclencheur, agent, action, résultat et au maximum une validation.");
      return;
    }
    const positions = Object.fromEntries(nodes.map((node, index) => [node.type, index]));
    if (!(positions.trigger < positions.agent && positions.agent < positions.action && positions.action < positions.result)) {
      toast.error("Ordonnez les blocs : déclencheur, agent, action, puis résultat.");
      return;
    }
    ...
    const automation: Automation = {
      ...
      trigger: schedule === "manual" ? "Déclenchement manuel" : schedule === "hourly" ? "Chaque heure" : schedule === "daily" ? "Chaque jour" : "Chaque semaine",
      ...
      nextRun: schedule === "manual" ? undefined : previous?.nextRun && Date.parse(previous.nextRun) > Date.now() ? previous.nextRun : new Date(Date.now() + (schedule === "hourly" ? 3_600_000 : schedule === "weekly" ? 604_800_000 : 86_400_000)).toISOString(),
      ...
      retryPolicy: { maximumAttempts, backoffSeconds },
...
function AutomationRunHistory({ automationId }: { automationId: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    automationService.runs(automationId)
      .then((items) => { if (!cancelled) setRuns(items); })
      .catch((reason) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "Historique indisponible"); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [automationId]);

  if (loading) return <div className="flex min-h-32 items-center justify-center"><LoaderCircle className="size-5 animate-spin text-indigo-500" /></div>;
  if (error) return <p className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-4 text-sm text-rose-500">{error}</p>;
  if (!runs.length) return <p className="py-8 text-center text-sm text-muted-foreground">Aucune exécution enregistrée.</p>;

  return <div className="space-y-3">{runs.map((run) => <details key={run.id} className="rounded-xl border p-4"><summary className="cursor-pointer list-none"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium">{new Date(run.createdAt).toLocaleString("fr-FR")}</p><p className="mt-1 text-xs text-muted-foreground">{run.totalTokens.toLocaleString("fr-FR")} tokens · {(run.totalCostNanoUsd / 1_000_000_000).toLocaleString("fr-FR", { style: "currency", currency: "USD", maximumFractionDigits: 6 })}</p></div><Badge className={run.status === "completed" ? "bg-emerald-500/10 text-emerald-500" : run.status === "failed" ? "bg-rose-500/10 text-rose-500" : "bg-amber-500/10 text-amber-500"}>{run.status}</Badge></div></summary><div className="mt-4 space-y-2 border-t pt-4">{run.errorMessage ? <p className="rounded-lg bg-rose-500/5 p-3 text-xs text-rose-500">{run.errorCode}: {run.errorMessage}</p> : null}{run.result ? <p className="whitespace-pre-wrap text-xs leading-5 text-muted-foreground">{run.result}</p> : null}{run.steps.map((step) => <div key={step.id} className="flex items-start justify-between gap-3 rounded-lg bg-muted/30 p-3 text-xs"><div><p className="font-medium">{step.position + 1}. {nodeLabels[step.nodeType]}</p>{step.errorMessage ? <p className="mt-1 text-rose-500">{step.errorMessage}</p> : null}</div><span className="font-mono text-muted-foreground">{step.status}</span></div>)}</div></details>)}</div>;
}
