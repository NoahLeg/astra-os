...
export function DashboardPage() {
  ...
  return <div className="space-y-6 md:space-y-8">
      ...
      <div className="relative grid gap-8 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-center">
        <div className="min-w-0">
          ...
          <div className="mt-6 grid max-w-xl grid-cols-2 gap-3 sm:flex">
            <GlassSurface variant="glass" className="p-4 py-3"><p className="font-mono text-[9px] uppercase tracking-[.14em] text-muted-foreground">Agents disponibles</p><p className="mt-1 font-mono text-lg font-medium text-foreground">{availableAgents}</p></GlassSurface>
            <GlassSurface variant="glass" className="p-4 py-3"><p className="font-mono text-[9px] uppercase tracking-[.14em] text-muted-foreground">Décisions en attente</p><p className="mt-1 font-mono text-lg font-medium text-primary">{pendingApprovals.length}</p></GlassSurface>
          </div>

          <GlassSurface variant="floating" className="mt-7 p-3">
            ...
            <div className="flex flex-col gap-3 border-t border-border/50 px-2 pt-3 sm:flex-row sm:items-center">
              <div className="flex flex-1 flex-wrap gap-2"><Badge className="border-border/60 bg-background/50"><FolderKanban className="size-3" />Contexte projet</Badge><Badge className="border-border/60 bg-background/50"><ShieldCheck className="size-3" />Validation humaine</Badge><Badge className="border-border/60 bg-background/50"><Bot className="size-3" />Multi-agents</Badge></div>
              <GlassButton onClick={() => void analyze()} disabled={analyzing}>{analyzing ? <LoaderCircle className="size-4 animate-spin" /> : <Play className="size-4 fill-current" />}Comprendre l'objectif</GlassButton>
            </div>
          </GlassSurface>
        </div>
      </div>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{stats.map((stat) => <GlassSurface key={stat.label} variant="glass" className="p-4 pt-5" style={{ "--metric-accent": stat.accent } as CSSProperties}><div className="flex items-start justify-between"><span className={`${stat.bg} ${stat.color} rounded-lg p-2.5`}><stat.icon className="size-4" /></span><span className="font-mono text-[9px] uppercase tracking-[.08em] text-muted-foreground">{stat.delta}</span></div><p className="mt-4 font-mono text-2xl font-medium">{stat.value}</p><p className="mt-1 text-xs text-muted-foreground">{stat.label}</p></GlassSurface>)}</div>

    <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
      <GlassSurface variant="glass" className="p-0"><Card><CardHeader className="flex-row items-center justify-between"><div><CardTitle>Objectifs en cours</CardTitle><CardDescription>Progression synchronisée entre vos agents.</CardDescription></div><Link href="/goals"><Button variant="ghost" size="sm">Tout voir<ChevronRight className="size-4" /></Button></Link></CardHeader><CardContent className="space-y-3">{activeGoals.length ? activeGoals.map((goal) => <Link key={goal.id} href={`/goals/${goal.id}`} className="block rounded-lg border bg-background/60 p-4 transition hover:-translate-y-0.5 hover:border-primary/45"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="size-2 rounded-full bg-[#3A4CE0]" /><h3 className="truncate font-medium">{goal.title}</h3></div><p className="mt-1 truncate text-xs text-muted-foreground">Échéance {new Date(goal.dueDate).toLocaleDateString("fr-FR", { day: "numeric", month: "long" })} · Autonomie N{goal.autonomyLevel}</p></div><span className="font-mono text-sm font-medium">{goal.progress}%</span></div><Progress value={goal.progress} className="mt-4" /><div className="mt-3 flex items-center justify-between"><div className="flex -space-x-2">{goal.agentIds.slice(0, 5).map((id) => <span key={id} className="flex size-7 items-center justify-center rounded-full border-2 border-card bg-muted font-mono text-[8px] font-medium uppercase">{id.slice(0, 2)}</span>)}</div><span className="text-xs text-muted-foreground">{goal.steps.length} étapes planifiées</span></div></Link>) : <p className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">Aucun objectif actif.</p>}</CardContent></Card></GlassSurface>
      <GlassSurface variant="glass" className="p-0"><Card><CardHeader><CardTitle>Productivité augmentée</CardTitle><CardDescription>Heures réellement mesurées après exécution.</CardDescription></CardHeader><CardContent><div className="flex items-end justify-between"><div><p className="font-mono text-3xl font-medium">0 h</p><p className="mt-1 text-xs text-muted-foreground">Aucune mesure disponible</p></div><TimerReset className="size-8 text-cyan-500" /></div><div className="mt-5 h-36"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData}><defs><linearGradient id="productivity" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#6E42D9" stopOpacity={0.48} /><stop offset="100%" stopColor="#6E42D9" stopOpacity={0} /></linearGradient></defs><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#8589B8" }} /><Tooltip contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12 }} /><Area type="monotone" dataKey="value" stroke="#8C9AFF" strokeWidth={2} fill="url(#productivity)" /></AreaChart></ResponsiveContainer></div></CardContent></Card></GlassSurface>
    </div>

    <div className="grid gap-5 xl:grid-cols-3">
      <GlassSurface variant="glass" className="p-0"><Card><CardHeader className="flex-row items-center justify-between"><div><CardTitle>Validations requises</CardTitle><CardDescription>Décisions qui restent sous votre contrôle.</CardDescription></div><Link href="/approvals"><Badge className="border-[#FF4FA3]/20 bg-[#FF4FA3]/10 text-[#FF75B7]">{pendingApprovals.length} en attente</Badge></Link></CardHeader><CardContent className="space-y-3">{pendingApprovals.length ? pendingApprovals.slice(0, 2).map((approval) => <div key={approval.id} className="rounded-lg border bg-background/60 p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium">{approval.action}</p><p className="mt-1 text-xs text-muted-foreground">Demandé par {approval.agent}</p></div><RiskBadge risk={approval.risk} /></div></div>) : <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Aucune décision en attente.</p>}</CardContent></Card></GlassSurface>
      <GlassSurface variant="glass" className="p-0"><Card><CardHeader><CardTitle>Agents actifs</CardTitle><CardDescription>Travail disponible dans votre espace.</CardDescription></CardHeader><CardContent className="space-y-2">{activeAgents.length ? activeAgents.map((agent) => <Link key={agent.id} href={`/agents/${agent.id}`} className="flex items-center gap-3 rounded-lg p-2.5 hover:bg-muted"><span className="flex size-9 items-center justify-center rounded-lg bg-indigo-500/10 text-indigo-500"><Bot className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-sm font-medium">{agent.name}</p><AgentStatus status={agent.status} label={agent.lastActivity} /></div><ConfidenceIndicator value={agent.successRate} compact /></Link>) : <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Aucun agent actif.</p>}</CardContent></Card></GlassSurface>
      <GlassSurface variant="glass" className="p-0"><Card><CardHeader><CardTitle>Automatisations proposées</CardTitle><CardDescription>Opportunités détectées dans votre routine.</CardDescription></CardHeader><CardContent className="space-y-3">{automations.filter((item) => item.status === "suggested").length ? automations.filter((item) => item.status === "suggested").map((automation) => <div key={automation.id} className="rounded-lg border bg-background/60 p-4"><span className="mb-3 inline-flex rounded-lg bg-[#FF4FA3]/10 p-2 text-[#FF4FA3]"><Lightbulb className="size-4" /></span><p className="text-sm font-medium">{automation.name}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{automation.description}</p><Link href="/automations"><Button variant="outline" size="sm" className="mt-3">Examiner<ArrowRight className="size-3" /></Button></Link></div>) : <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Aucune proposition actuellement.</p>}</CardContent></Card></GlassSurface>
    </div>

    <div className="grid gap-5 lg:grid-cols-[1fr_.75fr]">
      <GlassSurface variant="glass" className="p-0"><Card><CardHeader><CardTitle>Activité récente</CardTitle><CardDescription>Un journal transparent de l'exécution numérique.</CardDescription></CardHeader><CardContent className="space-y-1">{activities.length ? activities.slice(0, 4).map((event) => <div key={event.id} className="flex items-center gap-3 border-b py-3 last:border-0"><span className="flex size-8 items-center justify-center rounded-lg bg-muted"><Zap className="size-3.5 text-indigo-500" /></span><div className="min-w-0 flex-1"><p className="truncate text-sm">{event.action}</p><p className="font-mono text-[9px] text-muted-foreground">{event.agent} · {event.timestamp}</p></div><ConfidenceIndicator value={event.confidence} compact /></div>) : <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Aucune activité enregistrée.</p>}</CardContent></Card></GlassSurface>
      <GlassSurface variant="glass" className="p-0"><Card><CardHeader><CardTitle>À venir</CardTitle><CardDescription>Échéances issues de vos objectifs réels.</CardDescription></CardHeader><CardContent className="space-y-3">{upcomingGoals.length ? upcomingGoals.map((goal) => { const dueDate = new Date(goal.dueDate); return <Link href={`/goals/${goal.id}`} key={goal.id} className="flex gap-3 rounded-lg border bg-background/60 p-3 transition hover:border-primary/40"><div className="flex w-14 shrink-0 flex-col items-center justify-center rounded-lg bg-muted font-mono text-[9px] font-medium uppercase text-indigo-500">{dueDate.toLocaleDateString("fr-FR", { day: "2-digit", month: "short" })}</div><div className="min-w-0"><p className="truncate text-sm font-medium">{goal.title}</p><p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground"><CalendarClock className="size-3" />Progression {goal.progress} %</p></div></Link>; }) : <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Aucune échéance planifiée.</p>}</CardContent></Card></GlassSurface>
    </div>
  </div>;
}