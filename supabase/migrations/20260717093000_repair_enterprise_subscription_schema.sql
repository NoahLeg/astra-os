...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'workspace_subscriptions_member_limit_override_check'
  ) then
    alter table public.workspace_subscriptions
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'subscription_plans_max_members_check'
  ) then
    alter table public.subscription_plans
...
values (
  'enterprise',
  'Entreprise',
  'Pour deployer Astra a l''echelle d''une organisation avec des sieges et quotas contractuels.',
  0,
  50000,
  3000,
  180,
